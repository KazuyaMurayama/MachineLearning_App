...
| リポ | 役割 | 主要ファイル |
|------|------|------------|
| `freelance-compass` | 上流：事業戦略・意思決定（6エージェント） | `outputs/recommendation.md` (v29.0)、`tasks.md` |
...
- 目標: 最初の 5 社パイロット契約（freelance-compass の Full Go 条件 = 有償診断 ¥30k × 2件）
...
| `claude/review-repo-docs-2nBuQ` | 戦略整合性レビュー・tasks.md 再構成 |

## 重要な不変条件（全員必読）

0. 🚨 **戦略 vs 実装の整合性**: freelance-compass v29.0 は「EC + クリニック」、本リポは「士業 + EC」。**ターゲットが異なる**。確定するまで GTM 加速は不可。詳細は `tasks.md` の P0。
1. **`docs/sales-assets/pricing-and-scope.md` が料金の SSOT**: 他ファイルに料金を書く場合は必ず整合を取る。なお freelance-compass v29.0 ¥180k/月とも乖離あり（要決定）。
2. **ec-what-if の R²=-0.16**: 応急処置 `np.clip(0.0, 0.95)` + 警告バナーのみ。**本質的な精度は未改善**。予測値を確定値として顧客提示禁止。対応は `docs/sales-assets/objection-handling.md` D-2/D-3。
...
- **上流リポ**: `freelance-compass/outputs/recommendation.md` (v29.0)、`freelance-compass/outputs/session-summary-WDemK.md`、`freelance-compass/tasks.md`

## よくある質問

**Q. 新しいアプリを追加したい**
→ `.claude/commands/mvp-plan.md` → `mvp-build.md` のフローに従う。サンプルデータ生成スクリプト + `apps/<name>/app.py` + `requirements.txt` + `packages.txt` の 4 点セット。**ただしターゲットが士業/クリニック どちらかは P0 決定後に確定。**

**Q. 営業資料の料金を変更したい**
→ `pricing-and-scope.md` を先に更新 → 他ファイルを整合させる。「Standard 月額」「パイロット期間合計」を混同しない。freelance-compass v29.0 ¥180k/月 とも要整合。

**Q. セッションがタイムアウトした**
→ 上記「API Stream idle timeout 対策」を厳格に適用。1 回の送信サイズを半分以下にし、`create_or_update_file` に切り替える。

**Q. 「士業」と「クリニック」どちらに集中すべきか**
→ `tasks.md` P0 の未決事項。ユーザーの戦略判断が必要。決定までは士業アプリ 11本を凍結せず、新規クリニック開発も先行投資しない。
