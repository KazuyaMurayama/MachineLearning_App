...
def build_alert_df(df):
    """最新月の顧問先別アラート一覧を構築"""
    latest_month = df["請求年月"].max()
    latest = df[df["請求年月"] == latest_month].copy()
    ...
    # 悪化傾向フラグ: 直近3ヶ月平均が当月より5日以上悪い
    latest["悪化傾向"] = (latest["直近3ヶ月平均遅延"] > latest["入金遅延日数"] + 5).map({True: "⚠️ 悪化", False: ""})
...
# === KPIカード ===
latest_month = df["請求年月"].max()
latest_df = df[df["請求年月"] == latest_month].copy()
...
total_overdue = latest_df[latest_df["入金遅延日数"] > 0]["月額顧問料"].sum()
avg_delay = latest_df[latest_df["入金遅延日数"] > 0]["入金遅延日数"].mean()

kc1, kc2, kc3, kc4 = st.columns(4)
kc1.metric("🔴 危険（16日以上）", f"{n_danger}件", f"全{n_total}件中")
kc2.metric("🟡 注意（6-15日）", f"{n_warning}件", f"全{n_total}件中")
kc3.metric("💰 遅延中の顧問料合計", f"¥{total_overdue:,.0f}", "当月")
...
# ─────────────────────────────────────────
with tab1:
    st.header(f"📋 アラート一覧 — {latest_month}")

    alert_df, _ = build_alert_df(df)
    ...
    st.download_button(
        "📥 アラート一覧をCSVダウンロード",
        csv_data,
        f"payment_alert_{latest_month}.csv",
        "text/csv",
...
with tab2:
    ...
    with col_l:
        ...
        ax1.set_title(f"遅延分類内訳（{latest_month}）", fontsize=12, fontweight="bold")
        plt.tight_layout()
        st.pyplot(fig1)
        plt.close(fig1)
...
誠に恐れ入りますが、**○月○日**までにご入金いただけない場合、顧問契約の継続につきまして改めてご相談させていただく場合がございます。
