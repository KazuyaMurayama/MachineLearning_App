...
ACTION_TEMPLATES={
    ...
    "累計購入金額":(None,None,0),"平均注文単価":(None,None,0),
...
for k,v in {"churn_model":None,"churn_trained":False,"churn_le":None,"churn_feat":None,
    ...
    "churn_sv":None,"churn_Xs":None,"churn_stats":None,
...
def _train_churn(df):
    with warnings.catch_warnings():
        ...
        st.session_state.update({"churn_model":m,"churn_le":le,"churn_trained":True,"churn_feat":fc,
            "churn_Xte":Xte,"churn_yte":yte,"churn_metrics":metrics,"churn_sv":sv,"churn_Xs":Xs,"churn_stats":stats})

def _train_demand(df):
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        tc="売上金額"
        fc=[c for c in df.columns if c not in [tc,"日付","SKU"]]
        dw=df.drop(columns=["日付","SKU"],errors="ignore")
        dp,le=preprocess_data(dw.copy(),tc,is_training=True)
        Xtr,Xte,ytr,yte=split_data(dp.copy(),tc)
        ...
        st.session_state.update({"demand_model":m,"demand_trained":True,"demand_le":le,"demand_metrics":metrics})
...
if up2:
    ndf2=load_file(up2)
    if ndf2 is not None and "売上金額" in ndf2.columns:
...
st.sidebar.caption("MVP #2 — EC向け離脱予測デモ v0.1")

# === Main ===
st.title("🛒 EC顧客離脱予測AI")
st.markdown("LightGBM + SHAP + **逆SHAP** で、顧客の離脱リスクを予測し、**「何を変えれば離脱を防げるか」**を具体的に提案します。")
tab1,tab2,tab3,tab4,tab5,tab6=st.tabs(["📊 離脱リスク一覧","📈 売上/需要予測","📉 モデル評価","🔍 SHAP要因分析","💡 逆SHAP改善提案","📋 データプレビュー"])
...
with tab1:
    ...
    if st.session_state.churn_trained and st.session_state.churn_df is not None:
        ...
        show=["顧客ID","リスクレベル","離脱確率","直近購入からの経過日数","過去90日購入回数","メール開封率","カート放棄回数","決済失敗回数"]
        show=[c for c in show if c in ds.columns]
        st.dataframe(ds.head(10)[show],use_container_width=True,hide_index=True)
...
with tab2:
    ...
    if st.session_state.demand_trained and st.session_state.demand_df is not None:
        ...
        st.markdown(f"**{ddf['SKU'].nunique()} SKU × {ddf['日付'].nunique()} 日間**の売上データで学習済み")
        dm=st.session_state.demand_metrics
        c1,c2,c3=st.columns(3)
        c1.metric("MAE",f"¥{dm['MAE']:,.0f}"); c2.metric("RMSE",f"¥{dm['RMSE']:,.0f}"); c3.metric("R²",f"{dm['R2']:.3f}")
