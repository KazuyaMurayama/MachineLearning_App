...
このスクリプトは import 時にも自動実行される（__name__ ガード無し）。
app.py の train_models() でデータ読込前に import される設計。
"""
import numpy as np
import pandas as pd
import os
...
_DATA_PATH = os.path.join(_SCRIPT_DIR, "sample_data", "training_data.csv")
_SCRIPT_PATH = os.path.abspath(__file__)

# スクリプトが CSV より新しい場合のみ再生成
_need_regen = not os.path.exists(_DATA_PATH)
if not _need_regen:
    try:
        _need_regen = os.path.getmtime(_SCRIPT_PATH) > os.path.getmtime(_DATA_PATH)
    except OSError:
        _need_regen = True
...
if _need_regen:
    ...
    out_dir = os.path.join(_SCRIPT_DIR, "sample_data")
