...
st.markdown("""
<style>
.tier-card{border-radius:12px;padding:24px;text-align:center;height:100%;}
...
.hero-section{text-align:center;padding:40px 0 30px 0;}
.hero-section h1{font-size:2.2rem;margin-bottom:8px;}
.hero-section p{font-size:1.1rem;color:#475569;line-height:1.7;}
.credential-item{padding:12px 16px;border-left:4px solid #6366F1;margin-bottom:12px;background:#F8FAFC;border-radius:0 8px 8px 0;}
...
def render_service_tiers():
    ...
    for i,(key,t) in enumerate(SERVICE_TIERS.items()):
        with cols[i]:
            ...
            st.markdown(f"""
<div class="tier-card" style="border:2px solid {t['color']};background:linear-gradient(180deg,{t['color']}08,{t['color']}03);">
...
def render_result(score,level,biz_type,cat_scores):
    ...
    st.markdown("📩 **お問い合わせ**: 上記の診断結果をスクリーンショットしてお送りいただくか、直接ご連絡ください。")
...
def render_diagnostic():
    ...
    st.markdown("---")
    # 業種選択
    biz_type=st.radio("貴社の業種を選択してください",["士業","EC"],horizontal=True,key="biz_type")
    ...
    for i,q in enumerate(questions):
        ...
        ans=st.radio(f"q{i+1}",q["opts"],key=f"q_{i}",label_visibility="collapsed",horizontal=False)
    ...
    if st.button("📊 診断結果を見る",type="primary",use_container_width=True):
        ...
        st.session_state["diag_level"]=level
        st.session_state["diag_biz"]=biz_type
        st.session_state["diag_cats"]=cat_scores
...
# === Session State ===
for k,v in {"diag_done":False,"diag_score":0,"diag_level":None,"diag_biz":"士業","diag_cats":{}}.items():
