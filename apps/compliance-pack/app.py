...
.kpi-card .kpi-value{font-size:2.4rem;font-weight:bold;color:#059669;margin:8px 0 4px 0;}
...
<h1>🛡️ AI導入、守秘義務は大丈夫ですか？</h1>
<p>守秘義務契約・データ規程・AI同意書の3点セットを5分で準備完了。</p>
</div>
""",unsafe_allow_html=True)
...
_eboxcss='style="background:linear-gradient(135deg,#F0FDF4,#DCFCE7);border:1px solid #86EFAC;border-radius:12px;padding:20px;margin:4px 0;"'
_etitlecss='style="font-size:0.9rem;color:#64748B;margin-bottom:4px;"'
_evalcss='style="font-size:1.2rem;font-weight:700;color:#059669;margin:4px 0;"'
_ededcss='style="font-size:0.8rem;color:#94A3B8;"'
with _bf1:
    st.markdown(f'<div {_eboxcss}><div {_etitlecss}>❌ Before</div><div {_evalcss}>弁護士に依頼して2週間</div><div {_ededcss}>高コスト・長期間の準備</div></div>',unsafe_allow_html=True)
with _bf2:
    st.markdown(f'<div {_eboxcss}><div {_etitlecss}>✅ After</div><div {_evalcss}>5分で3点セット完成</div><div {_ededcss}>情報入力だけで即生成</div></div>',unsafe_allow_html=True)
with _bf3:
    st.markdown(f'<div {_eboxcss}><div {_etitlecss}>📈 年間効果</div><div {_evalcss}>準備コスト80%削減</div><div {_ededcss}>何度でも修正・再生成可能</div></div>',unsafe_allow_html=True)
