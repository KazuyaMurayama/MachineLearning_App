...
# === 固有データ: 横断分析用（RFMセグメント × チャネル × ROI） ===
cross_records = []
for seg in ["Champions","Loyal","AtRisk","Lost","New"]:
    for ch in channels:
        cross_records.append({
            "RFMセグメント": seg, "チャネル": ch,
            "売上": np.random.randint(500000, 5000000),
            "広告費": np.random.randint(100000, 1500000),
            "離脱率": round(np.random.uniform(0.05, 0.35), 3),
        })
cross_df = pd.DataFrame(cross_records)
cross_df["ROI"] = (cross_df["売上"] / cross_df["広告費"]).round(2)
...
cross_df.to_csv(os.path.join(out_dir, "cross_segment.csv"), index=False, encoding="utf-8-sig")
